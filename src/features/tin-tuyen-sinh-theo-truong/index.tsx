...
export const SearchResultsBySchool = () => {
  const router = useRouter();
  const { page } = router.query;

  const schoolQuery = (router.query.slug as string) || "all";
  const pageQuery = (page as string) || "1";

  const { data, isLoading, isError } = useQuery(
    ["filter_common_admission", `${pageQuery}`, `${schoolQuery}`],
    () =>
      getAdmissions({
  ...
  const next = () => {
    router.push(
      `/tin-tuyen-sinh-theo-truong/${schoolQuery}?page=${
    ...
    <>
      <Box background={"linear-gradient(90deg,#360350,#66008D)"}>
        <Container maxW={"6xl"} py={4}>
          ...
          {schoolQuery && <SearchForm schoolOther={schoolQuery} />}
        </Container>
      </Box>
